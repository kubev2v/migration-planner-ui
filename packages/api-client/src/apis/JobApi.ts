...
import { JobFromJSON, JobToJSON } from "../models/index.js";
...
export interface JobApiInterface {
   ...
   * @memberof JobApiInterface
   */
  createRVToolsAssessmentRaw(
    requestParameters: CreateRVToolsAssessmentRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<Job>>;
...
export class JobApi extends runtime.BaseAPI implements JobApiInterface {
  ...
  async cancelJobRaw(
    ...
    const response = await this.request(
      {
        path: `/api/v1/assessments/jobs/{id}`.replace(
          `{${"id"}}`,
   ...
   * Create an assessment from RVTools file asynchronously
   */
  async createRVToolsAssessmentRaw(
    requestParameters: CreateRVToolsAssessmentRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<Job>> {
