...
import { PresignedUrlFromJSON, PresignedUrlToJSON } from "../models/index.js";
...
export interface ImageApiInterface {
   ...
   */
  getSourceDownloadURLRaw(
    requestParameters: GetSourceDownloadURLRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<PresignedUrl>>;
   ...
   */
  headImageRaw(
    requestParameters: HeadImageRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
...
export class ImageApi extends runtime.BaseAPI implements ImageApiInterface {
  ...
  async getSourceDownloadURLRaw(
    ...
    const response = await this.request(
      {
        path: `/api/v1/sources/{id}/image-url`.replace(
          `{${"id"}}`,
  ...
  async headImageRaw(
    ...
    const response = await this.request(
      {
        path: `/api/v1/sources/{id}/image`.replace(
          `{${"id"}}`,
          encodeURIComponent(String(requestParameters["id"])),
        ),
        method: "HEAD",
